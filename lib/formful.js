...
formful.createServer = function (resources, options, handler) {
  ...
  restful.extendRouter(router, resources, options, function (req, res, status, key, value) {
    ...
    //
    //console.log(req.url, req.restful.action)
    view[req.restful.action].render({ data: value, resource: req.restful.resource });
    res.end(view[req.restful.action].present({ data: value, resource: req.restful.resource }));
