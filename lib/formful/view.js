...
//
var views = ['create', 'destroy', 'update', 'show', 'list', 'layout'],
    templates = {};
...
module['exports'] = {
  ...
  create: function (resource, options, callback) {
    ...
    Object.keys(resource.schema.properties).forEach(function (property) {
      ...
      if (options.errors) {
        options.errors.forEach(function (e) {
          //
          // If there is an error on one of our properties,
          // then set error
          //
...
function dropdown (input) {
  ...
  str = "<div class=`control-group`>"
           ...
           str += "<option> Please select " + input.name + "</option>"
            input.value.forEach(function(v, i){
